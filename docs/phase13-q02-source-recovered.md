...
7. **`HackhubPost` relocated to `content/`, per §1 of `docs/phase13-quest-structure-standard.md`.** `HackhubPost` had been missed on the original pass: it stayed as an inline object literal in all four quest files (Q01/Q02 × production/replay). Fixed by adding `Q0N_HACKHUB_POST_PRODUCTION`/`_REPLAY` to `content/q01.ts` / `content/q02.ts`; quest files now only reference them. Also revised the `HackhubPost` copy itself: Q01's original text nearly duplicated its incoming mail's body — replaced with a short teaser ("Short audit for a client in Jakarta. Details in your mail.") matching Q02's already-teaser-style post (revised to "Follow-up from the last client. Check your mail." for exact parallel structure). Two magic-string duplicates of `Q01_REPORT_SUBJECT`'s literal value (`Bank.transaction`'s `description`, and the `OnComplete` reply's subject) were also replaced with `Q01_REPORT_SUBJECT` / `` `Re: ${Q01_REPORT_SUBJECT}` ``, matching Q02's existing pattern. Q01's single-thread mail-subject pattern (opening brief and report share one subject) was evaluated against Q02's separate-subject pattern and kept as-is — see `docs/phase13-quest-structure-standard.md` §2 for the resulting Q03-Q16 standard (Q02's pattern) and the rationale for not retrofitting Q01.
8. **CRI gateway hostname/private IP now render a connection-timeout page instead of the native 404.** Browsing `cri-gateway.internal` or `10.42.7.18` previously hit HackHub's generic "no such host" 404 on both `http://` and `https://`, since neither had a registered `Website`. Explored as a design-review artifact mockup (three candidate page styles compared side by side, one built out with a full CSS boot-sequence animation and exported as a reference GIF) before committing to an approach. Implemented the "Diagnostic overlay" option as the new shared, reusable `src/infrastructure/hackhub/websites/templates/unreachable-diagnostic.html` template (see `docs/phase13-quest-structure-standard.md` §9 for the full template catalog, including the "Perimeter notice" option kept in reserve for a future quest) and wired it into two new `Website` registrations in `q02-cri-gateway-portal.ts` — one for the hostname (needs `Network.registerDomain(Q02_HIDDEN_HOSTNAME, Q02_HIDDEN_HOSTNAME_IP)`, added to both quest files' `OnStart`/`OnComplete`/`OnAbandon`) and one for the raw IP (no domain registration needed, matching `Q02GatewayWebsite`'s existing raw-IP pattern). No protocol-gating — the page renders identically on `http://` and `https://`, since being unreachable doesn't depend on protocol. Not yet live-tested in game.
9. **`Q02EdgeWebsite` was serving identical content on `http://` and `https://` — now gated per the mandatory protocol rule locked the same day (`docs/phase13-quest-structure-standard.md` §6).** Port 80 is absent from `Q02_NMAP_RESULT` for this target (only 22/443/8443 modeled); an absent port now defaults to `CLOSE`, so plain HTTP must return 400 Bad Request. Fixed by converting `q02-edge-portal.ts` to a `DynamicWebsitePageDefinition` with a new `q02-edge-http-error.html`, matching `Q01SkynetLogisticsWebsite`'s and `Q02GatewayWebsite`'s existing pattern exactly. Not yet live-tested in game.
10. **Q01's incoming mail unified between production and replay.** It had its own `_PRODUCTION`/`_REPLAY` split (added alongside the item-7 `content/` relocation) even though the wording had no functional reason to differ — unlike `Q01_COMPLETION_MAIL_CONTENT` (differs because production mentions a real money reward), the opening brief said the same thing either way, and the "DEV REPLAY" signal is already carried by `Title`/`HackhubPost`/the `"Adrian Cole [DEV]"` author. Flagged by the user as inconsistent with `Q02_INCOMING_MAIL_CONTENT`, which was never split. Fixed by merging into one shared `Q01_INCOMING_MAIL_CONTENT` (production's wording), removing the replay-only "DEV REPLAY — Q01 TEST CONTRACT" opening line and "This mail belongs to the development replay fixture." closing line entirely — replay now sends the byte-identical mail production does.
11. **`Q02_INCOMING_MAIL_CONTENT` / `Q02_INCOMING_MAIL_SUBJECT` wording revised.** Subject changed `"One more thing"` → `"Quick follow-up"`. Body restructured with an explicit opening line tying back to Q01's completion ("Got a small follow-up from the client after your last report.") followed by a short `FOLLOW-UP` block (`Client:`/`Host:`/`Note:`), still closing with the same non-alarming cover framing as before ("Might just be something old they forgot about..."). Purely a wording/structure change — `Q02_ADRIAN_EMAIL`, the host value, and the mail-read gating in `q02-quest.ts`/`dev/q02-replay-quest.ts` (which reference the constant, not a literal) are unaffected.
12. **Adrian's email domain changed, and the character/organization email contract revised.** `adrian.cole@entityresolution.lock` → `adrian.cole@phantom-net.void` — the old domain read as a meta reference to the mod's own title. Full discussion and rationale in `docs/email-character-contract.md`'s "Revision history" (also summarized in `docs/phase13-quest-structure-standard.md` §10): personal/individual contacts now use `.void`; organization mailboxes were corrected to follow that organization's own website TLD (`.idx`) instead of sharing the personal-contact TLD. Surfaced and fixed two pre-existing flavor-text addresses on Q01's static pages that predated the contract: `edge-gateway@skynet-logistics.idx` (already correct — unchanged) and `security-review@skynet-logistics` (missing its TLD — fixed to `security-review@skynet-logistics.idx`). `ADRIAN_COLE.email` in `content/characters.ts` is the single source of truth (`Q01_ADRIAN_EMAIL`/`Q02_ADRIAN_EMAIL` derive from it), so no quest logic changed.
