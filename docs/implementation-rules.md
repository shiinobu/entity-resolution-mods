...
## 11. Source comment policy — MANDATORY RULE: zero comments in `src/`, always

**No comments anywhere in `src/`** — `content/`, `infrastructure/hackhub/` (including shared/reusable utilities like `logger.ts`, `dev-flag.ts`, `scheduled-callback.ts`, not just per-quest files), `application/`, `domain/`, `state/`, `core/` — at any point in development, not just once a quest reaches FINAL LOCK. A dedicated file already exists for this; there is no reason to write `//` anywhere in source.

1. **While a quest is under active development** (claimed, being built or reworked, not yet FINAL LOCK): any "why" a developer would normally write inline instead goes into `docs/temp-notes.md` — a generic, reusable scratch file (its own header states which quest it's currently scoped to; never name a quest in the filename itself).
2. **Once that quest reaches FINAL LOCK** (design and live-test cycle done, no further behavior changes expected): filter `docs/temp-notes.md`'s entries for that quest — for each one that records a real decision, bug, SDK quirk, or rationale not already covered, add/extend the matching entry in `docs/bugs.md` (engine/SDK bugs and quirks) or the quest's section in `docs/source-current.md` (design/story/objective rationale) — never a new doc file. Entries that were just noise get dropped. The file then empties out (or gets reused for whichever quest becomes actively developed next).
3. Verify with `grep -rn "^\s*//" src/` (and `grep -rn "console\."`, see §12) — zero matches expected, at any point, not just at FINAL LOCK.

**Supersedes the original 2026-09-17 wording of this section**, which allowed "normal in-progress commenting... while a quest's design is still settling" — that carve-out is removed as of later the same day: "buat RULES WAJIB - TIDAK BOLEH ADA COMMENT DI SOURCE PROJECT, SUDAH DISEDIAKAN FILE TERSENDIRI." Applied to Q03 2026-09-17 (see `docs/bugs.md` entries 18-24, `docs/source-current.md`'s Q03 section, and `docs/temp-notes.md` for what was extracted/is pending).
