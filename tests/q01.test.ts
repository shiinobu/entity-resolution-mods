...
describe("Q01 — THE CONTRACT (FINAL LOCK, live-in-game passed)", () => {
    ...
    it("shares Q01_NMAP_RESULT, Q01_LYNX_RESULT, and Q01_NETWORK_PORTS from content/q01.ts instead of duplicating them in the quest file", () => {
    ...
    it("registers a GoMail compose template for the audit report, and validates its raw JSON field payload", () => {
    ...
    it("declares the completion mail once in content/q01.ts instead of duplicating the literal in the quest file", () => {
    ...
    it("declares HackhubPost and the incoming mail in content/q01.ts instead of as inline literals in the quest file — a teaser that points to the mail, not a near-duplicate of its body", () => {
    ...
    it("sends the incoming mail body as-is, no separate dev-only variant", () => {
...
describe("Q01 — web path boundary (experimental path-based redesign, not yet locked)", () => {
